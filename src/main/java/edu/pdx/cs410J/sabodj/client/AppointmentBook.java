...
public class AppointmentBook extends AbstractAppointmentBook<Appointment> {
     ...
     *
     * @param startTime is the time the appointments must start after
     * @param endTime is the time the appointments but end before
     ...
     */
    public ArrayList<Appointment> getApptsInRange(Date startTime, Date endTime) throws ParseException, IllegalStateException {
        ...
        for(int i = 0; i < appts.size(); i++){
            ...
            if(appt.getBeginTime().compareTo(startTime) >= 0 && appt.getEndTime().compareTo(endTime) <= 0 ) {
                list.add(appt);
            }
        }
